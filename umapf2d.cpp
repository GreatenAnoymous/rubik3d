...
struct flowNode{
    ...
    flowNode(Location _u,Location _v,double _t){
        u=_u;
        v=_v;
        t=_t;
    }
...
Paths retrieve_paths(capacityList&capacity,residualList &residual_capacity,Graph &graphx,Configs &starts,
...
void constructGraph(Configs &starts,Configs &goals,int xmax,int ymax,Obstacles &obstacles,int timeSteps,
    Graph &graphx,IdNodeMap &id_node,
    NodeIdMap& node_id,
    Graph::vertex_descriptor &source_id,Graph::vertex_descriptor &sink_id)
{
    std::unordered_set<Location> reachable_vertices;
    ...
    for(int t=0;t<timeSteps;t++){
        std::unordered_set<Location> next_reachable_vertices;
        for(auto v1:reachable_vertices){
            std::vector<Location> neighbors={Location(v1.x+1,v1.y),Location(v1.x,v1.y+1),
                                             Location(v1.x-1,v1.y),Location(v1.x,v1.y-1)};
            flowNode node1(v1,v1,t);
            for(auto v2:neighbors){
                if(v2.x<0||v2.y<0||v2.x>=xmax||v2.y>=ymax) continue;
                if(obstacles.find(v2)!=obstacles.end()) continue;
            
                insert_node(id_node,node_id,node1,id,graphx);
                std::vector<Location> edge_node;
    ...
    flowNode source(Location(0,0),Location(xmax,xmax),0);
    flowNode sink(Location(ymax,ymax),Location(0,0),0);
...
Paths umapf_solve(Configs &starts, 
    ...
    while(true){
        ...
        try{
            ...
            constructGraph(starts,goals,xmax,ymax,obstacles,timeSteps,graphx,id_node,node_id,source,sink);
            ...
            if(flow==starts.size()){
                Paths paths=retrieve_paths(capacity,residual_capacity,graphx,starts,id_node,node_id,sink,goals);
                return paths;
            }
            else{
                timeSteps++;
    ...
    for(int i=0;i<starts.size();i++){
        ...
        while(current_id!=sink_id){
            ...
            if(fabs(timestep-int(timestep))<1e-2){
                auto node=id_node[current_id].u;
                path.push_back(node);
            }
